...
2) 以下の手順で実装を開始 / 続行する:
   2.1) **着手を決めた時点で、まず該当 issue の status を `In Progress` に変更する**（`linear issue update <ID> --state "In Progress"`）。これは sub agent に実装を渡す前に、この手順の最初に必ず行う（作業中であることを Linear 上で可視化するため。In Progress を飛ばして直接 In Review にしない）。その上で issue のやり取りをもとに実装・検証・テスト計画を立てる。設計レベルで更新した方がよければ積極的に更新する。
